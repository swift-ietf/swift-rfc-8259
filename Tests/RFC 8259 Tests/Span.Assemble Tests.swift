...
struct SpanAssembleTests {
    ...
    @Test
    func `Assemble.from output matches direct Decode.Implementation output`() throws {
        // Round-trip: the assembler's output via Assemble.from MUST
        // equal the direct Decode.Implementation.parse(_:) output on the
        // same bytes. This pins the FAST PATH's semantics against the
        // existing wholesale-parser surface.
        let bytes: [UInt8] = Swift.Array(#"{"a":1,"b":[true,null,"s"]}"#.utf8)
        try bytes.withUnsafeBufferPointer { (buf: UnsafeBufferPointer<UInt8>) throws(RFC_8259.Error) in
            let span = buf.span
            // Direct parse.
            let direct = try RFC_8259.Decode.Implementation.parse(span, maxDepth: 512)
